...
export default function LandingUIDesign() {
  ...
  return (
    <div className="vetscue-obsidian-theme min-h-screen bg-[#131313] text-[#e5e2e1] font-body overflow-x-hidden">
      ...
      {/* ── Top Navigation ── */}
      <header className="fixed top-0 w-full z-[100] bg-[#131313]/80 backdrop-blur-xl border-b border-white/5">
        <div className="flex justify-between items-center px-5 sm:px-8 py-4 max-w-7xl mx-auto">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl sm:text-2xl font-black text-[#76d6d5] tracking-tighter font-headline leading-none">VetsCue</span>
          </div>
          ...
          <button 
            ...
            className="md:hidden p-2 rounded-xl bg-white/5 border border-white/10 text-[#76d6d5]"
          >
      ...
      <div 
        className={`md:hidden fixed inset-0 bg-[#131313]/20 backdrop-blur-[2px] z-[200] transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setMobileMenuOpen(false)}
      />
      
      {/* Mobile Sidebar Content - 70% width for visibility of background */}
      <div className={`md:hidden fixed top-0 right-0 w-[70%] sm:w-[320px] h-full bg-[#1c1b1b] border-l border-white/5 z-[210] transition-transform duration-500 ease-in-out flex flex-col p-8 ${mobileMenuOpen ? 'translate-x-0 shadow-[-30px_0_80px_rgba(0,0,0,0.8)]' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-[#76d6d5] text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>pets</span>
            <span className="text-xl font-black text-[#76d6d5] font-headline tracking-tighter">VetsCue</span>
          </div>
          <button onClick={() => setMobileMenuOpen(false)} className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#76d6d5] border border-white/10 hover:bg-white/10 transition-all">
        ...
        <div className="space-y-3 mb-8">
          ...
          <Link onClick={() => setMobileMenuOpen(false)} to="/login" className="flex items-center gap-4 py-4 px-5 rounded-2xl bg-white/5 border border-white/5 text-[#e5e2e1] font-headline font-bold text-sm hover:bg-white/10 transition-all">
            <span className="material-symbols-outlined text-[#76d6d5]">login</span>
            Login
          </Link>
          <Link onClick={() => setMobileMenuOpen(false)} to="/register" className="flex items-center gap-4 py-4 px-5 rounded-2xl bg-[#76d6d5] text-[#131313] font-headline font-black text-sm shadow-lg shadow-[#76d6d5]/10 group transition-all">
        ...
        <div className="mt-auto border-t border-white/5 pt-8 text-center sm:text-left">
          <p className="text-[10px] font-black uppercase tracking-widest text-white/20 mb-1 leading-none">VetsCue Sanctuary</p>
          <p className="text-[10px] text-white/10 leading-none">Built for Life · 2026</p>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <div className="space-y-3">
              <span className="text-[#76d6d5] text-[10px] font-black uppercase tracking-[0.4em]">Choose who you are</span>
              <h2 className="font-headline font-bold text-4xl text-[#e5e2e1] tracking-tight">Login Portals</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-8">
              {[
                ...
                <div key={label} className="text-center space-y-2">
                  <p className="text-5xl font-black text-[#76d6d5] tracking-tighter">{value > 1000 ? `${(value/1000).toFixed(1)}k+` : value}</p>
      ...
      <section className="px-6 py-20 bg-[#131313]">
        <div className="max-w-4xl mx-auto glass-card rounded-[3rem] border border-[#76d6d5]/20 bg-[#1c1b1b] p-16 text-center space-y-8">
          ...
          <div className="relative flex flex-col sm:flex-row gap-4 items-center justify-center">
            <Link to="/register" className="h-16 px-10 bg-[#76d6d5] text-[#131313] rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-[0_20px_40px_-10px_rgba(118,214,213,0.3)]">
                Join the Rescue
              </Link>
              <Link to="/fundraisers" className="h-16 px-10 rounded-2xl border border-white/10 text-[#e5e2e1] font-black uppercase tracking-widest text-xs flex items-center justify-center hover:bg-white/5 transition-all">
                Donate to Help
              </Link>
          </div>
        </div>
