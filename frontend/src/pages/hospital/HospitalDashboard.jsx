    ...
    if (!user.isApproved) return (
        <div className="resqpet-obsidian-theme flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6">
            ...
            <div className="space-y-2">
                ...
                <p className="text-on-surface/40 max-w-sm">Your hospital account is under admin review. You'll be notified once approved.</p>
            </div>
        </div>
    );
    ...
    if (locationRequired) return (
        <div className="resqpet-obsidian-theme w-full text-on-surface space-y-8">
            <section className="space-y-2">
                ...
                <p className="text-on-surface/40">{locationError || 'Location is required to view nearby escalations.'}</p>
            </section>
            <div className="glass-card rounded-[2rem] border border-surface-border bg-surface p-6 space-y-4 max-w-md">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <input value={manualLat} onChange={(e) => setManualLat(e.target.value)} placeholder="Latitude"
                        className="rounded-2xl border border-surface-border bg-transparent px-4 py-3 text-xs text-on-surface placeholder:text-on-surface/30 focus:outline-none focus:border-[#76d6d5]/40" />
                ...
                <button onClick={handleManualSave} disabled={locationSaving}
                    className="w-full py-3 rounded-2xl border border-surface-border text-on-surface/40 text-xs font-black uppercase tracking-widest hover:text-[#76d6d5] hover:border-[#76d6d5]/20 transition-all disabled:opacity-50">
                    Save Coordinates
                </button>
            </div>
        </div>
        ...
        <div className="resqpet-obsidian-theme w-full text-on-surface space-y-8">
            ...
            <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1">
                    ...
                    <p className="text-on-surface/40 text-sm">Escalated cases requiring hospital admission near you.</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="glass-card rounded-2xl border border-surface-border bg-surface px-5 py-3 text-center">
                        <p className="text-2xl font-headline font-black text-[#76d6d5]">{cases.length}</p>
                        <p className="text-[8px] font-black uppercase tracking-widest text-on-surface/30">Active</p>
                    </div>
                    <button onClick={fetchData}
                        className="h-12 w-12 rounded-2xl bg-surface-hover border border-surface-border flex items-center justify-center hover:bg-surface-hover transition-all">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    ...
                    <Link key={to} to={to}
                        ...
                        <span className="text-xs font-black uppercase tracking-wider text-on-surface/60 group-hover:text-on-surface transition-colors">{label}</span>
                    </Link>
                ))}
            </div>
            ...
            {/* Cases */}
            {loading ? (
                <div className="space-y-4">{[1, 2, 3].map(i => <div key={i} className="h-40 rounded-[2rem] bg-surface-hover animate-pulse" />)}</div>
            ) : cases.length === 0 ? (
                <div className="glass-card rounded-[2.5rem] border border-dashed border-surface-border p-16 text-center space-y-4">
                    ...
                    <button onClick={fetchData} className="rounded-2xl border border-surface-border px-6 py-2.5 text-xs font-black uppercase tracking-widest text-on-surface/40 hover:text-[#76d6d5] hover:border-[#76d6d5]/20 transition-all">
                        Refresh
                    </button>
                </div>
            ) : (
                <div className="space-y-5">
                    {cases.map((c) => (
                        <div key={c._id} className="glass-card rounded-[2rem] border border-surface-border bg-surface p-6 space-y-4 hover:border-surface-border transition-all">
                            {/* Case Header */}
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1 space-y-1.5">
                                    <p className="font-bold text-on-surface truncate">{c.description}</p>
                                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-on-surface/30">
