        ...
        >
            ...
            <div className={`px-3 py-2.5 mx-3 mt-3 rounded-btn border flex items-center gap-2 transition-all ${
                ...
                <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                    isNewUI ? 'bg-brand/10' : 'bg-surface-hover'
                }`}>
                    <RoleIcon className={`w-3.5 h-3.5 ${isNewUI ? 'text-brand' : 'text-on-background'}`} />
                </div>
                <div className="min-w-0">
                    <p className={`text-xs font-semibold truncate leading-tight ${isNewUI ? 'text-on-background' : 'text-on-background'}`}>{user?.name}</p>
                    <p className={`text-[10px] leading-tight ${isNewUI ? 'text-on-background/50' : 'text-on-background/70'}`}>{roleLabels[user?.role]}</p>
                </div>
            </div>
                ...
                {links.map(({ to, label, Icon }) => {
                    ...
                    <Component
                        ...
                        className={(navProps) => {
                            ...
                            if (isNewUI) {
                                return active
                                    ? `${base} bg-brand text-white shadow-[0_10px_20px_rgba(var(--brand-primary-rgb),0.3)]`
                ...
                {showComingSoon && (
                    <div className="mt-3">
                        <div className="flex items-center gap-2 px-2 mb-1">
                            <p className={`text-[11px] uppercase tracking-widest font-bold ${isNewUI ? 'text-brand' : 'text-surface-muted'}`}>
                                Coming Soon
                            </p>
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
                                isNewUI ? 'bg-brand/10 text-brand border border-brand/20' : 'bg-surface-hover border-surface-border text-on-background'
                            }`}>
                                Phase 2
                            </span>
                        </div>
                        {comingSoon.map(({ tag, label }) => (
                            ...
                            >
                                ...
                                <LockClosedIcon className="w-3 h-3 text-surface-muted ml-auto flex-shrink-0" />
                            </button>
                        ))}
                    </div>
                )}
            ...
            <div className={`px-4 py-4 border-t flex items-center justify-between ${isNewUI ? 'border-surface-border' : 'border-surface-border'}`}>
                <div className="flex flex-col">
                    <p className={`text-[9px] uppercase tracking-widest font-black ${isNewUI ? 'text-on-background/20' : 'text-surface-muted'}`}>
                        VetsCue Platform
                    </p>
                    <p className={`text-[10px] ${isNewUI ? 'text-on-background/40 font-medium' : 'text-text-muted'}`}>
